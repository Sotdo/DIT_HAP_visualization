...
query_genes = st.sidebar.text_area(
    "Enter the gene name or systematic ID", "SPAC1002.09c\nSPAC3G9.12")

st.sidebar.markdown("---")

for query in query_genes.split("\n"):
    
    igene = st.container()
    sysID, gene_col = display_basic_information(igene, query, merged_gene_info)

    insertion_GMs, gene_level_GMs, insertion_last_tp = get_insertions_in_genes(query, insertion_annotations, insertion_LFCs, gene_level_LFCs, timepoints)
